...
static inline def_EHelper(gp1) {
  switch (s->isa.ext_opcode) {
    EXW(0,add,s->dest.width) EXW(1,or, s->dest.width) EXW(2,adc,s->dest.width) EXW(3,sbb,s->dest.width)
    EXW(4,and,s->dest.width) EXW(5,sub,s->dest.width) EXW(6,xor,s->dest.width) EXW(7,cmp,s->dest.width)
  }
}
...
static inline def_EHelper(gp2) {
  switch (s->isa.ext_opcode) {
    EXW(0,rol,s->dest.width) EXW(1,ror,s->dest.width) EMPTY(2) EMPTY(3)
    EXW(4,shl,s->dest.width) EXW(5,shr,s->dest.width) EMPTY(6) EXW(7,sar,s->dest.width)
  }
}
...
static inline def_EHelper(gp3) {
  switch (s->isa.ext_opcode) {
    IDEXW(0,test_I,test,s->dest.width) EMPTY(1) EX(2,not) EX(3,neg)
    EX(4,mul) EX(5,imul1)/*possible I_E2G*/ EX(6,div) EX(7,idiv)
  }
}
...
static inline def_EHelper(gp4) {
 // printf("%x\n",s->isa.ext_opcode);
  switch (s->isa.ext_opcode) {
    EXW(0,inc,s->dest.width) EXW(1,dec,s->dest.width) EMPTY(2) EMPTY(3)
...
static inline def_EHelper(2byte_esc) {
  ...
  /* TODO: Add more instructions!!! */
    ...
    IDEX(0xbd,E2G,bsr)
     ...
     IDEX(0xac,Ib_G2E,shrd)//may have problems
    default: exec_inv(s);
  }
}
 ...
 // if(cpu.pc==0x3032366)printf("%x\n",opcode);
  ...
  switch (opcode) {
   ...
   IDEXW(0xec,in_dx2a,in,1)
    ...
    EX(0x61,popa)
    EX(0xcf,iret)
    case 0xf3:cpu.pc+=3;s->seq_pc+=3;break;
